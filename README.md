...
1. 确保MySQL服务已启动
2. 打开MySQL命令行或使用MySQL Workbench连接到MySQL服务器
3. 执行以下SQL命令创建数据库并初始化数据：
...
CREATE TABLE `team_club` (
  ...
  `manager_id` bigint(20) NOT NULL COMMENT '关联sys_user.id (俱乐部管理员)',
  `description` text,
...
CREATE TABLE `club_league_relation` (
  ...
  CONSTRAINT `fk_club_league_club` FOREIGN KEY (`club_id`) REFERENCES `team_club` (`id`),
...
CREATE TABLE `player_club_apply` (
  ...
  CONSTRAINT `fk_player_club_apply_club` FOREIGN KEY (`club_id`) REFERENCES `team_club` (`id`)
...
CREATE TABLE `fan_team_relation` (
  ...
  CONSTRAINT `fk_fan_team_club` FOREIGN KEY (`club_id`) REFERENCES `team_club` (`id`)
...
4. 启动前端开发服务器：
   ...
   npm run dev
   ```
5. 前端服务将运行在 `http://localhost:5173`（如果端口被占用，会自动使用其他端口）
...
2. 使用以下测试账号登录：
   ...
   - 球员：username: player1, password: 123456
   - 球迷：username: fan1, password: 123456
   - 俱乐部管理员：username: club1, password: 123456
